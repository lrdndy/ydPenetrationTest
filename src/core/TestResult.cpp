...
namespace ydtest {
static const char* nameOf(Outcome o){ return o==Outcome::Pass?"PASS":(o==Outcome::Fail?"FAIL":"SKIP"); }
...
void TestResult::add(Outcome o,const std::string& n,const std::string& d){ checks_.push_back({o,n,d}); log_.info("RESULT",std::string(nameOf(o))+" | "+n+(d.empty()?"":" | "+d)); }
void TestResult::pass(const std::string& n,const std::string& d){add(Outcome::Pass,n,d);} void TestResult::fail(const std::string& n,const std::string& d){add(Outcome::Fail,n,d);} void TestResult::skip(const std::string& n,const std::string& d){add(Outcome::Skip,n,d);}
