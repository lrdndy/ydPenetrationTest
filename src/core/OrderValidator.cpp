...
ValidationResult OrderValidator::instrument(YDApi* api,const std::string& id) const {
    ...
    if(!api->getInstrumentByID(id.c_str())) { log_.error("VALIDATION","invalid instrument: "+id); return {false,"instrument not found"}; }
    return {true,""};
}
ValidationResult OrderValidator::limitPrice(const YDInstrument* i,double p) const {
    ...
    if(std::fabs(units-nearest)>1e-7){ std::ostringstream s; s<<"price="<<p<<" is not multiple of Tick="<<i->Tick; log_.error("VALIDATION",s.str()); return {false,s.str()}; }
    return {true,""};
}
ValidationResult OrderValidator::limitVolume(const YDInstrument* i,int v) const {
    ...
    if(v<i->MinLimitOrderVolume || v>i->MaxLimitOrderVolume || (i->MinLimitOrderVolume>0 && v%i->MinLimitOrderVolume!=0)){
        std::ostringstream s; s<<"volume="<<v<<" allowed=["<<i->MinLimitOrderVolume<<','<<i->MaxLimitOrderVolume<<"] step="<<i->MinLimitOrderVolume; log_.error("VALIDATION",s.str()); return {false,s.str()};
    }
    return {true,""};
}
}
